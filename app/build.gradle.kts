...
android {
  ...
  defaultConfig {
    ...
    buildConfigField("String", "SIGNAL_CDSI_URL", "\"https://cdsi.red.local\"")
    buildConfigField("String", "SIGNAL_SERVICE_STATUS_URL", "\"uptime.red.local\"")
    buildConfigField("String", "SIGNAL_SVR2_URL", "\"https://svr2.red.local\"")
    buildConfigField("String", "SIGNAL_SFU_URL", "\"https://sfu.voip.red.local\"")
    buildConfigField("String", "SIGNAL_STAGING_SFU_URL", "\"https://sfu.staging.voip.red.local\"")
    buildConfigField("String[]", "SIGNAL_SFU_INTERNAL_NAMES", "new String[]{\"Test\", \"Staging\", \"Development\"}")
    buildConfigField("String[]", "SIGNAL_SFU_INTERNAL_URLS", "new String[]{\"https://sfu.test.voip.red.local\", \"https://sfu.staging.voip.red.local\", \"https://sfu.staging.test.voip.red.local\"}")
    buildConfigField("String", "CONTENT_PROXY_HOST", "\"contentproxy.red.local\"")
    ...
    buildConfigField("String", "BADGE_STATIC_ROOT", "\"https://updates2.red.local/static/badges/\"")
  ...
  productFlavors {
    ...
    create("website") {
      ...
      buildConfigField("String", "APK_UPDATE_MANIFEST_URL", "\"https://updates.red.local/android/latest.json\"")
    ...
    create("staging") {
      ...
      buildConfigField("String", "SIGNAL_CDN3_URL", "\"https://cdn3-staging.red.local\"")
      buildConfigField("String", "SIGNAL_CDSI_URL", "\"https://cdsi.staging.red.local\"")
      buildConfigField("String", "SIGNAL_SVR2_URL", "\"https://svr2.staging.red.local\"")
...
fun isGitAvailable(): Boolean {
  return file("${project.rootDir}/.git").exists()
}

fun getLastCommitTimestamp(): String {
  if (!isGitAvailable()) return System.currentTimeMillis().toString()
  return try {
    providers.exec {
      commandLine("git", "log", "-1", "--pretty=format:%ct")
      isIgnoreExitValue = true
    }.standardOutput.asText.get().trim().ifEmpty { System.currentTimeMillis().toString() } + "000"
  } catch (e: Exception) {
...
fun getGitHash(): String {
  if (!isGitAvailable()) return "unknown"
  return try {
    providers.exec {
...
fun getLastCommitDateTimeUtc(): String {
  val defaultDate = DateTimeFormatter.ofPattern("MMM d '@' HH:mm 'UTC'", Locale.US).withZone(ZoneOffset.UTC).format(Instant.now())
  if (!isGitAvailable()) return defaultDate

  return try {
    val timestampStr = providers.exec {
      commandLine("git", "log", "-1", "--pretty=format:%ct")
      isIgnoreExitValue = true
    }.standardOutput.asText.get().trim()

    if (timestampStr.isEmpty()) return defaultDate
